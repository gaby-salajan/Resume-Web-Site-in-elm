...
monthsBetween dA dB =
    case compare dA dB of
        LT ->   if (month dA) /= Nothing && (month dB) /= Nothing then
                    case compareMonth (Maybe.withDefault Jan (month dA)) (Maybe.withDefault Jan (month dB)) of
                        LT -> Just (abs ((monthsBetweenMonths (Maybe.withDefault Jan (month dA)) (Maybe.withDefault Jan (month dB))) + (year dB - year dA) * 12) )
                        EQ -> Just ((year dB - year dA) * 12)
                        GT -> Just (abs ((monthsBetweenMonths (Maybe.withDefault Jan (month dA)) (Maybe.withDefault Jan (month dB))) - (year dB - year dA) * 12) )  
                else
                    if (month dA) == Nothing && (month dB) == Nothing then
                        Just (((year dB) - (year dA)) * 12)
                    else
                        Nothing

        EQ ->   if (month dA) /= Nothing && (month dB) /= Nothing then
                    Just 0
                else
                    Nothing

        GT ->   if (month dA) /= Nothing && (month dB) /= Nothing then
                    case compareMonth (Maybe.withDefault Jan (month dA)) (Maybe.withDefault Jan (month dB)) of
                        LT -> Just (abs ((monthsBetweenMonths (Maybe.withDefault Jan (month dA)) (Maybe.withDefault Jan (month dB))) - (year dA - year dB) * 12) )
                        EQ -> Just ( (year dB - year dA) * 12 )
                        GT -> Just ( ((monthsBetweenMonths (Maybe.withDefault Jan (month dA)) (Maybe.withDefault Jan (month dB))) - (year dA - year dB) * 12) )  
                else
                    if (month dA) == Nothing && (month dB) == Nothing then
                        Just (((year dA) - (year dB)) * 12)
                    else
                        Nothing
...
compare (Date d1) (Date d2) =
    case Basics.compare d1.year d2.year of
        LT -> LT
        EQ ->   if d1.month /= Nothing && d2.month /= Nothing then
                    compareMonth (Maybe.withDefault Dec d1.month) (Maybe.withDefault Dec d2.month)
                else
...
view : Date -> Html msg
view (Date d) =
    if d.month == Nothing then
        div [] 
        [
            p [class "year", style "display" "inline-block"] [text  (String.fromInt d.year)]
        ]
    else
        div [] 
        [
            p [class "month", style "display" "inline-block"] [text (monthToString (Maybe.withDefault Jan d.month))],
            p [style "width" "2%", style "display" "inline-block"] [text " "],
            p [class "year", style "display" "inline-block"] [text  (String.fromInt d.year)]
        ]
